...
{
	if (en)
		bq2560x_set_value(BQ2560X_REG_01, BQ2560X_REG01_CHG_CONFIG_MASK,
			  BQ2560X_REG01_CHG_CONFIG_SHIFT, BQ2560X_REG01_CHG_ENABLE);
	else
		bq2560x_set_value(BQ2560X_REG_01, BQ2560X_REG01_CHG_CONFIG_MASK,
			  BQ2560X_REG01_CHG_CONFIG_SHIFT, BQ2560X_REG01_CHG_DISABLE);
...
static int bq2560x_set_chg_current(struct charger_device *chg_dev, u32 cur)
{
	...
	bq2560x_set_eoc_current(chg_dev, 150000);
	return 0;
}
...
void bq2560x_set_highz(bool en)
{
	if (en)
		bq2560x_set_value(BQ2560X_REG_00, BQ2560X_REG00_ENHIZ_MASK,
				BQ2560X_REG00_ENHIZ_SHIFT, BQ2560X_REG00_HIZ_ENABLE);
	else
		bq2560x_set_value(BQ2560X_REG_00, BQ2560X_REG00_ENHIZ_MASK,
				BQ2560X_REG00_ENHIZ_SHIFT, BQ2560X_REG00_HIZ_DISABLE);
}
...
static int bq2560x_set_input_current(struct charger_device *chg_dev, u32 limit)
{
	...
	if (limit == 0) {
		bq2560x_set_highz(true);
		bq2560x_read_reg(BQ2560X_REG_00, &reg_read);
		pr_info("chgr limit: %d, set highz. reg0: 0x%x\n", limit, reg_read);
	} else {
		bq2560x_read_reg(BQ2560X_REG_00, &reg_read);
		pr_info("chgr limit: %d. highz reg0: 0x%x\n", limit, reg_read);

		if (reg_read & BQ2560X_REG00_ENHIZ_MASK) {
			bq2560x_set_highz(false);
			bq2560x_read_reg(BQ2560X_REG_00, &reg_read);
			pr_info("set highz false, new reg0: 0x%x\n", reg_read);
		}
	}
	bq2560x_set_value(BQ2560X_REG_00, BQ2560X_REG00_IINLIM_MASK,
			  BQ2560X_REG00_IINLIM_SHIFT, reg_value);
...
{
	if (enable) {
		bq2560x_set_value(BQ2560X_REG_01, BQ2560X_REG01_OTG_CONFIG_MASK,
				  BQ2560X_REG01_OTG_CONFIG_SHIFT, BQ2560X_REG01_OTG_ENABLE);
		schedule_delayed_work(&bq2560x_data->otg_feed_watchdog_work,
				msecs_to_jiffies(50));
	} else {
		bq2560x_set_value(BQ2560X_REG_01, BQ2560X_REG01_OTG_CONFIG_MASK,
				  BQ2560X_REG01_OTG_CONFIG_SHIFT, BQ2560X_REG01_OTG_DISABLE);
...
static int bq2560x_set_rechg_voltage(struct charger_device *chg_dev, u32 vol)
{
	u8 reg_value = 0;
...
static u8 bq2560x_reg[BQ2560X_REG_NUM] = { 0 };
...
static int bq2560x_dump_register(struct charger_device *chg_dev)
{
	...
	for (i = 0; i < BQ2560X_REG_NUM; i++) {
		bq2560x_read_reg(i, &bq2560x_reg[i]);
		pr_info("[bq2560x reg@][0x%x]=0x%.2x ", i, bq2560x_reg[i]);
	}
	pr_info("\n");

...
static int hl7019x_write_reg(u8 reg, u8 val)
{
	...
	if (ret < 0)
		dev_err(&hl7019x_data->client->dev, "error=%d\n", ret);
...
{
	if (en)
		hl7019x_set_value(HL7019X_REG_01, HL7019X_REG01_CHG_CONFIG_MASK,
			  HL7019X_REG01_CHG_CONFIG_SHIFT, HL7019X_REG01_CHG_ENABLE);
	else
		hl7019x_set_value(HL7019X_REG_01, HL7019X_REG01_CHG_CONFIG_MASK,
			  HL7019X_REG01_CHG_CONFIG_SHIFT, HL7019X_REG01_CHG_DISABLE);
...
static int hl7019x_set_chg_current(struct charger_device *chg_dev, u32 cur)
{
	...
	hl7019x_set_value(HL7019X_REG_02, HL7019X_REG02_ICHG_MASK,
			  HL7019X_REG02_ICHG_SHIFT, reg_value);

	hl7019x_set_eoc_current(chg_dev, 150000);
	return 0;
}
...
void hl7019x_set_highz(bool en)
{
	if (en)
		hl7019x_set_value(HL7019X_REG_00, HL7019X_REG00_ENHIZ_MASK,
				HL7019X_REG00_ENHIZ_SHIFT, HL7019X_REG00_HIZ_ENABLE);
	else
		hl7019x_set_value(HL7019X_REG_00, HL7019X_REG00_ENHIZ_MASK,
				HL7019X_REG00_ENHIZ_SHIFT, HL7019X_REG00_HIZ_DISABLE);
}

static int hl7019x_set_input_current(struct charger_device *chg_dev, u32 limit)
{
	u8 reg_value = 0, reg_read = 0;

	limit /= 1000;

	if (limit == 0) {
		hl7019x_set_highz(true);
		hl7019x_read_reg(HL7019X_REG_00, &reg_read);
		pr_info("chgr limit: %d, set highz. reg0: 0x%x\n", limit, reg_read);
	} else {
		hl7019x_read_reg(HL7019X_REG_00, &reg_read);
		pr_info("chgr limit: %d. highz reg0: 0x%x\n", limit, reg_read);

		if (reg_read & HL7019X_REG00_ENHIZ_MASK) {
			hl7019x_set_highz(false);
			hl7019x_read_reg(HL7019X_REG_00, &reg_read);
			pr_info("set highz false, new reg0: 0x%x\n", reg_read);
		}
	}
	...
	hl7019x_set_value(HL7019X_REG_00, HL7019X_REG00_IINLIM_MASK,
			  HL7019X_REG00_IINLIM_SHIFT, reg_value);
...
{
	if (enable) {
		hl7019x_set_value(HL7019X_REG_01, HL7019X_REG01_OTG_CONFIG_MASK,
				  HL7019X_REG01_OTG_CONFIG_SHIFT, HL7019X_REG01_OTG_ENABLE);
		schedule_delayed_work(&hl7019x_data->otg_feed_watchdog_work,
				msecs_to_jiffies(50));
	} else {
		hl7019x_set_value(HL7019X_REG_01, HL7019X_REG01_OTG_CONFIG_MASK,
				  HL7019X_REG01_OTG_CONFIG_SHIFT, HL7019X_REG01_OTG_DISABLE);
...
#define HL7019X_REG_NUM 12
static u8 hl7019x_reg[HL7019X_REG_NUM] = { 0 };
...
static int hl7019x_dump_register(struct charger_device *chg_dev)
{
	...
	for (i = 0; i < HL7019X_REG_NUM; i++) {
		hl7019x_read_reg(i, &hl7019x_reg[i]);
		pr_info("[hl7019x reg@][0x%x]=0x%.2x ", i, hl7019x_reg[i]);
	}
	pr_info("\n");

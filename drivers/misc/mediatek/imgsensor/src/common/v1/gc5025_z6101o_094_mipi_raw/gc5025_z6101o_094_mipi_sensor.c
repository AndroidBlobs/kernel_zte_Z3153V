 ...
 * Filename:
 * ---------
 *     GC5025_Z6101O_094mipi_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "gc5025_z6101o_094_mipi_sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC5025_Z6101O_094_camera_sensor"
#define LOG_1 LOG_INF("GC5025_Z6101O_094,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static kal_bool GC5025_Z6101O_094DuringTestPattern = KAL_FALSE;

static kal_uint32 Dgain_ratio = 256;
static kal_bool DR_State = KAL_TRUE;

static imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC5025_Z6101O_094_SENSOR_ID,        /*record sensor id defined in Kd_imgsensor.h*/
...
typedef struct otp_gc5025_z6101o_094 {
	...
	kal_uint16 reg_num;
} gc5025_z6101o_094_otp;

static gc5025_z6101o_094_otp gc5025_z6101o_094_otp_info;
...
static kal_uint8 gc5025_z6101o_094_read_otp(kal_uint8 addr)
{
	kal_uint8 value;
	kal_uint8 regd4;
...
static void gc5025_z6101o_094_read_otp_group(kal_uint8 addr, kal_uint8 *buff, int size)
{
	kal_uint8 i;
	kal_uint8 regd4;
	kal_uint16 realaddr = addr * 8;
...
static void gc5025_z6101o_094_select_page_otp(otp_page otp_select_page)
{
	kal_uint8 page;
...
static void gc5025_z6101o_094_gcore_read_otp_info(void)
{
	...
	memset(&gc5025_z6101o_094_otp_info, 0, sizeof(gc5025_z6101o_094_otp));

	/*TODO*/
	gc5025_z6101o_094_select_page_otp(otp_page0);
	flagdd = gc5025_z6101o_094_read_otp(0x00);
	LOG_INF("GC5025_Z6101O_094_OTP_DD : flag_dd = 0x%x\n", flagdd);
	flag_chipversion = gc5025_z6101o_094_read_otp(0x7f);

	/*DD*/
	switch (flagdd & 0x03) {
	case 0x00:
		LOG_INF("GC5025_Z6101O_094_OTP_DD is Empty !!\n");
		gc5025_z6101o_094_otp_info.dd_flag = 0x00;
		break;
	case 0x01:
		LOG_INF("GC5025_Z6101O_094_OTP_DD is Valid!!\n");
		total_number = gc5025_z6101o_094_read_otp(0x01) + gc5025_z6101o_094_read_otp(0x02);
		LOG_INF("GC5025_Z6101O_094_OTP : total_number = %d\n", total_number);
		if (total_number <= 31) {
			gc5025_z6101o_094_read_otp_group(0x03, &ddtempbuff[0], total_number * 4);
		} else {
			gc5025_z6101o_094_read_otp_group(0x03, &ddtempbuff[0], 31 * 4);
			gc5025_z6101o_094_select_page_otp(otp_page1);
			gc5025_z6101o_094_read_otp_group(0x29, &ddtempbuff[31 * 4], (total_number - 31) * 4);
		}

		/*DD check sum*/
		gc5025_z6101o_094_select_page_otp(otp_page1);
		ddchecksum = gc5025_z6101o_094_read_otp(0x61);
		check = total_number;
		for (i = 0; i < 4 * total_number; i++) {
			check += ddtempbuff[i];
		}
		if ((check % 255 + 1) == ddchecksum) {
			LOG_INF("GC5025_Z6101O_094_OTP_DD:DD checksum correct!checksum = 0x%x\n", ddchecksum);
		} else {
			LOG_INF("GC5025_Z6101O_094_OTP_DD:DD check sum error!otpchecksum = 0x%x, sum = 0x%x\n",
				ddchecksum, (check % 255 + 1));
		}

		for (i = 0; i < total_number; i++) {
			LOG_INF("GC5025_Z6101O_094_OTP_DD:index = %d,data[0] = %x,data[1] = %x,data[2]=%x,data[3]=%x\n",
			...
			if (ddtempbuff[4 * i + 3] & 0x10) {
				...
				case 3:
					for (j = 0; j < 4; j++) {
						gc5025_z6101o_094_otp_info.dd_param_x[cnt] =
							(((kal_uint16)ddtempbuff[4*i+1] & 0x0f) << 8)+ddtempbuff[4*i];
						gc5025_z6101o_094_otp_info.dd_param_y[cnt] =
							((kal_uint16)ddtempbuff[4*i+2] << 4) +
								((ddtempbuff[4*i+1] & 0xf0) >> 4) + j;
						gc5025_z6101o_094_otp_info.dd_param_type[cnt++] = 2;
					}
					break;
				case 4:
					for (j = 0; j < 2; j++) {
				...
				default:
					...
					gc5025_z6101o_094_otp_info.dd_param_type[cnt++] =
						ddtempbuff[4*i+3] & 0x0f;
					break;
				}
			} else {
				LOG_INF("GC5025_Z6101O_094_OTP_DD:check_id[%d] = %x,checkid error!!\n",
					i, ddtempbuff[4*i+3] & 0xf0);
			}
		}

		gc5025_z6101o_094_otp_info.dd_cnt = cnt;
		gc5025_z6101o_094_otp_info.dd_flag = 0x01;
		break;
	case 0x02:
	case 0x03:
		LOG_INF("GC5025_Z6101O_094_OTP_DD is Invalid !!\n");
		gc5025_z6101o_094_otp_info.dd_flag = 0x02;
		break;
	default:
		break;
	}
	...
	/*For Chip Version*/
	LOG_INF("GC5025_Z6101O_094_OTP_CHIPVESION:flag_chipversion = 0x%x\n", flag_chipversion);

	switch ((flag_chipversion>>4)&0x03) {
	case 0x00:
		LOG_INF("GC5025_Z6101O_094_OTP_CHIPVERSION is Empty !!\n");
		break;
	case 0x01:
		LOG_INF("GC5025_Z6101O_094_OTP_CHIPVERSION is Valid !!\n");
		gc5025_z6101o_094_select_page_otp(otp_page1);
		i = 0;
		do {
			gc5025_z6101o_094_otp_info.reg_addr[i] = gc5025_z6101o_094_read_otp(REG_ROM_START + i*2);
			gc5025_z6101o_094_otp_info.reg_value[i] = gc5025_z6101o_094_read_otp(REG_ROM_START + i*2 + 1);
			LOG_INF("GC5025_Z6101O_094_OTP_CHIPVERSION reg_addr[%d] = 0x%x,reg_value[%d] = 0x%x\n",
				i, gc5025_z6101o_094_otp_info.reg_addr[i], i, gc5025_z6101o_094_otp_info.reg_value[i]);
			i++;
		} while ((gc5025_z6101o_094_otp_info.reg_addr[i-1] != 0) && (i < 10));
		gc5025_z6101o_094_otp_info.reg_num = i - 1;
		break;
	case 0x02:
		LOG_INF("GC5025_Z6101O_094_OTP_CHIPVERSION is Invalid !!\n");
		break;
	default:
		break;
	}
...
static void gc5025_z6101o_094_gcore_update_dd(void)
{
	...
	if (gc5025_z6101o_094_otp_info.dd_flag == 0x01) {
	#if defined(GC5025_Z6101O_094_MIRROR_NORMAL)
	/*do nothing*/
	#elif defined(GC5025_Z6101O_094_MIRROR_H)
	for (i = 0; i < gc5025_z6101o_094_otp_info.dd_cnt; i++) {
		if (gc5025_z6101o_094_otp_info.dd_param_type[i] == 0) {
			gc5025_z6101o_094_otp_info.dd_param_x[i] =
				WINDOW_WIDTH-gc5025_z6101o_094_otp_info.dd_param_x[i]+1;
		} else if (gc5025_z6101o_094_otp_info.dd_param_type[i] == 1) {
			gc5025_z6101o_094_otp_info.dd_param_x[i] =
				WINDOW_WIDTH - gc5025_z6101o_094_otp_info.dd_param_x[i]-1;
		} else {
			gc5025_z6101o_094_otp_info.dd_param_x[i] =
				WINDOW_WIDTH - gc5025_z6101o_094_otp_info.dd_param_x[i];
		}
	}
	#elif defined(GC5025_Z6101O_094_MIRROR_V)
	for (i = 0; i < gc5025_z6101o_094_otp_info.dd_cnt; i++) {
		gc5025_z6101o_094_otp_info.dd_param_y[i] =
			WINDOW_HEIGHT - gc5025_z6101o_094_otp_info.dd_param_y[i] + 1;
	}
	#elif defined(GC5025_Z6101O_094_MIRROR_HV)
	for (i = 0; i < gc5025_z6101o_094_otp_info.dd_cnt; i++) {
		if (gc5025_z6101o_094_otp_info.dd_param_type[i] == 0) {
	...
	for (i = 0 ; i < gc5025_z6101o_094_otp_info.dd_cnt-1; i++) {
		for (j = i+1; j < gc5025_z6101o_094_otp_info.dd_cnt; j++) {
			if (gc5025_z6101o_094_otp_info.dd_param_y[i] > gc5025_z6101o_094_otp_info.dd_param_y[j]) {
					temp_x = gc5025_z6101o_094_otp_info.dd_param_x[i];
					gc5025_z6101o_094_otp_info.dd_param_x[i] =
						gc5025_z6101o_094_otp_info.dd_param_x[j];
					gc5025_z6101o_094_otp_info.dd_param_x[j] = temp_x;
					temp_y = gc5025_z6101o_094_otp_info.dd_param_y[i];
					gc5025_z6101o_094_otp_info.dd_param_y[i] =
						gc5025_z6101o_094_otp_info.dd_param_y[j];
					gc5025_z6101o_094_otp_info.dd_param_y[j] = temp_y;
					temp_type = gc5025_z6101o_094_otp_info.dd_param_type[i];
					gc5025_z6101o_094_otp_info.dd_param_type[i] =
						gc5025_z6101o_094_otp_info.dd_param_type[j];
					gc5025_z6101o_094_otp_info.dd_param_type[j] = temp_type;
			}
		}
	}
		...
		/*x*/
		for (i = 0; i < gc5025_z6101o_094_otp_info.dd_cnt; i++) {
			if (gc5025_z6101o_094_otp_info.dd_param_y[i] == gc5025_z6101o_094_otp_info.dd_param_y[i+1]) {
				s = i++;
				while ((gc5025_z6101o_094_otp_info.dd_param_y[s] ==
						gc5025_z6101o_094_otp_info.dd_param_y[i+1])
						&& (i < gc5025_z6101o_094_otp_info.dd_cnt-1))
					i++;
				e = i;

				for (n = s; n < e; n++) {
					for (m = n+1; m < e+1; m++) {
						if (gc5025_z6101o_094_otp_info.dd_param_x[n] >
							gc5025_z6101o_094_otp_info.dd_param_x[m]) {
							temp_x = gc5025_z6101o_094_otp_info.dd_param_x[n];
							gc5025_z6101o_094_otp_info.dd_param_x[n] =
								gc5025_z6101o_094_otp_info.dd_param_x[m];
							gc5025_z6101o_094_otp_info.dd_param_x[m] = temp_x;
							temp_y = gc5025_z6101o_094_otp_info.dd_param_y[n];
							gc5025_z6101o_094_otp_info.dd_param_y[n] =
								gc5025_z6101o_094_otp_info.dd_param_y[m];
							gc5025_z6101o_094_otp_info.dd_param_y[m] = temp_y;
							temp_type = gc5025_z6101o_094_otp_info.dd_param_type[n];
							gc5025_z6101o_094_otp_info.dd_param_type[n] =
								gc5025_z6101o_094_otp_info.dd_param_type[m];
							gc5025_z6101o_094_otp_info.dd_param_type[m] = temp_type;
						}
					}
				}
		...
		for (i = 0; i < gc5025_z6101o_094_otp_info.dd_cnt; i++) {
			temp_val0 = gc5025_z6101o_094_otp_info.dd_param_x[i] & 0x00ff;
			temp_val1 = (((gc5025_z6101o_094_otp_info.dd_param_y[i])<<4) & 0x00f0) +
				((gc5025_z6101o_094_otp_info.dd_param_x[i] >> 8) & 0X000f);
			temp_val2 = (gc5025_z6101o_094_otp_info.dd_param_y[i] >> 4) & 0xff;
			write_cmos_sensor(0xaa, i);
			write_cmos_sensor(0xac, temp_val0);
			write_cmos_sensor(0xac, temp_val1);
			write_cmos_sensor(0xac, temp_val2);
			while ((i < gc5025_z6101o_094_otp_info.dd_cnt - 1)
					&& (gc5025_z6101o_094_otp_info.dd_param_x[i] ==
					gc5025_z6101o_094_otp_info.dd_param_x[i+1])
					&& (gc5025_z6101o_094_otp_info.dd_param_y[i] ==
					gc5025_z6101o_094_otp_info.dd_param_y[i+1])) {
					flag = 1;
					i++;
			...
			if (flag)
				...
				write_cmos_sensor(0xac, gc5025_z6101o_094_otp_info.dd_param_type[i]);

			LOG_INF("GC5025_Z6101O_094_OTP_GC val0=0x%x,val1=0x%x,val2=0x%x\n",
				temp_val0, temp_val1, temp_val2);
			LOG_INF("GC5025_Z6101O_094_OTP_GC x=%d,y=%d\n",
...
static void gc5025_z6101o_094_gcore_update_chipversion(void)
{
	kal_uint8 i;

	LOG_INF("GC5025_Z6101O_094_OTP_UPDATE_CHIPVERSION:reg_num = %d\n",
		gc5025_z6101o_094_otp_info.reg_num);

	write_cmos_sensor(0xfe, 0x00);

	for (i = 0; i < gc5025_z6101o_094_otp_info.reg_num; i++) {
		write_cmos_sensor(gc5025_z6101o_094_otp_info.reg_addr[i], gc5025_z6101o_094_otp_info.reg_value[i]);
		LOG_INF("GC5025_Z6101O_094_OTP_UPDATE_CHIP_VERSION:{0x%x,0x%x}!!\n",
			gc5025_z6101o_094_otp_info.reg_addr[i], gc5025_z6101o_094_otp_info.reg_value[i]);
	}

	write_cmos_sensor(0xfe, 0x00);
}

static void gc5025_z6101o_094_gcore_update_otp(void)
{
	gc5025_z6101o_094_gcore_update_dd();
	gc5025_z6101o_094_gcore_update_chipversion();
}

static void gc5025_z6101o_094_gcore_enable_otp(otp_state state)
{
	...
	if (state)	{
		...
		LOG_INF("GC5025_Z6101O_094_OTP: Enable OTP!\n");
	} else {
		otp_en = otp_en & 0x7f;
		...
		LOG_INF("GC5025_Z6101O_094_OTP: Disable OTP!\n");
	}

}


static void gc5025_z6101o_094_gcore_initial_otp(void)
{
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xfe, 0x00);
...
static void gc5025_z6101o_094_gcore_identify_otp(void)
{
	gc5025_z6101o_094_gcore_initial_otp();
	gc5025_z6101o_094_gcore_enable_otp(otp_open);
	gc5025_z6101o_094_gcore_read_otp_info();
	gc5025_z6101o_094_gcore_enable_otp(otp_close);
}


static int gc5025_z6101o_094_gcore_check_version(void)
{
	kal_uint8 flag_doublereset = 0, flag_GC5025_Z6101O_094A = 0;
	...
	flag_doublereset = gc5025_z6101o_094_read_otp(0x26);
	flag_GC5025_Z6101O_094A = gc5025_z6101o_094_read_otp(0x27);

	if ((flag_GC5025_Z6101O_094A & 0x01) == 0x01) {
		LOG_INF("GC5025_Z6101O_094A sensor!\n");
		DR_State = KAL_FALSE;
	} else {
		if ((flag_doublereset & 0x03) == 0x01) {
			DR_State = KAL_FALSE;
			LOG_INF("GC5025_Z6101O_094 sensor double reset off\n");
		} else {
			DR_State = KAL_TRUE;
			LOG_INF("GC5025_Z6101O_094 sensor double reset on\n");
		}
	}
	gc5025_z6101o_094_gcore_enable_otp(otp_close);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if ((iReg >= ANALOG_GAIN_1) && (iReg < ANALOG_GAIN_2)) {
		...
		LOG_INF("GC5025_Z6101O_094MIPI analogic gain 1x, GC5025_Z6101O_094MIPI add pregain = %d\n", temp);
	} else {
		write_cmos_sensor(0xfe,  0x00);
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5025_Z6101O_094MIPI analogic gain 1.4x, GC5025_Z6101O_094MIPI add pregain = %d\n", temp);
	}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("gc5025_z6101o_094_front i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("gc5025_z6101o_094_front Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	gc5025_z6101o_094_gcore_check_version();
	/*Don't Remove!!*/
	gc5025_z6101o_094_gcore_identify_otp();
	/* initail sequence write in  */
	sensor_init();
	/*write registers from sram*/
	gc5025_z6101o_094_gcore_update_otp();
	...
	spin_unlock(&imgsensor_drv_lock);
	GC5025_Z6101O_094DuringTestPattern = KAL_FALSE;
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_INF("GC5025_Z6101O_094 normal video Entre\n");
...
UINT32 GC5025_Z6101O_094_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}    /*    GC5025_Z6101O_094MIPI_RAW_SensorInit    */
